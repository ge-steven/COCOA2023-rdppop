...
var cdnd_x3c = cy_x3c.compoundDragAndDrop();
var removeEmptyParents_x3c = true;

// custom handler to remove parents with only 1 child on drop
cy_x3c.on('cdndout', function(event, dropTarget){
  if( removeEmptyParents_x3c && isParentOfOneChild(dropTarget) ){
    removeParent(dropTarget);
  }
});

// custom handler to remove parents with only 1 child (on remove of drop target or drop sibling)
cy_x3c.on('remove', function(event){
  if( removeEmptyParents_x3c ){
    removeParentsOfOneChild(cy_x3c);
  }
});
...
let C =  [[1,2,3], [2,3,4], [1,2,5], [2,5,6], [1,5,6]]
var C_nodes = []
...
var colors = ['rgba(240, 186, 152, 0.5)', 'rgba(228, 46, 235, 0.2)', 'rgba(12, 40, 40, 0.5)', 'rgba(108, 62, 31, 0.6)', 'rgba(14, 233, 158, 0.3)']
var bubbles = []
